#!/usr/bin/env python
"""
Phase C: Main ML Training Script
Complete end-to-end training with dataset generation, hyperparameter tuning, and evaluation
"""
...
def main():
    """Run complete Phase C ML training pipeline."""
    
    logger.info("="*80)
    logger.info("PHASE C: MACHINE LEARNING MODEL TRAINING")
    logger.info("="*80)
    ...
    logger.info("Creating 10,000 sample dataset with class imbalance...")
    X_train, X_test, y_train, y_test = gen.generate_train_test_split(
        n_samples=10000,
        test_size=0.2,
        class_distribution={0: 0.40, 1: 0.25, 2: 0.20, 3: 0.15}  # Imbalanced
    )
    
    # Save datasets
    gen.save_dataset(X_train, y_train, str(data_dir / "X_train.npz"))
    gen.save_dataset(X_test, y_test, str(data_dir / "X_test.npz"))
    ...
    logger.info("\n[STEP 3] Tuning hyperparameters with GridSearchCV...")
    ...
    cv_results = trainer.cross_validate(X_train, y_train, cv=5, scoring='f1_weighted')
    ...
    logger.info(f"✓ Hyperparameters tuned via GridSearchCV")
    logger.info(f"✓ Cross-validation completed (5-fold)")
    logger.info(f"\nTest Set Metrics:")
